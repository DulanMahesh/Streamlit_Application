...
if uploaded_file is not None:
    ...
    if option == 'None':
        ...
        level = st.slider('level', 0, 5, 2) #minimum =0,maximum =5 ,default=2
        output = vignette(img, level)
    elif option == 'Pencil Sketch':
        ksize = st.slider('Blur kernel size', 1, 11, 5, step=2) #minimum =1,maximum =11 ,default=5, step has set to 2 because we need to have odd numbers for the kernal
